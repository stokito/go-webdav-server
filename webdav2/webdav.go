...
package webdav2 // import "golang.org/x/net/webdav"
...
func (h *Handler) handleLock(w http.ResponseWriter, r *http.Request) (retStatus int, retErr error) {
	/*	body := `<?xml version="1.0" encoding="utf-8"?>
		...
		<D:owner><D:href>STOKITO-SERV\Admin</D:href></D:owner>
		<D:timeout>Second-3600</D:timeout>
		<D:locktoken><D:href>1678266336</D:href></D:locktoken>
		<D:lockroot><D:href>/New Text Document.txt</D:href></D:lockroot>
		</D:activelock></D:lockdiscovery></D:prop>`*/

	token := "2678266336"
	w.Header().Set("Lock-Token", "<2678266336>")
	w.Header().Set("Content-Type", "application/xml; charset=utf-8")
	w.WriteHeader(http.StatusOK)
	ld := LockDetails{
		Root:      r.URL.Path,
		Duration:  3600,
		OwnerXML:  "STOKITO-SERV\\Admin",
		ZeroDepth: false,
	}
...
func (h *Handler) handleLockOrig(w http.ResponseWriter, r *http.Request) (retStatus int, retErr error) {
...
func (h *Handler) handleUnlockOrig(w http.ResponseWriter, r *http.Request) (status int, err error) {
	// http://www.webdav.org/specs/rfc4918.html#HEADER_Lock-Token says that the
...
func (h *Handler) handlePropfindOrig(w http.ResponseWriter, r *http.Request) (status int, err error) {
	reqPath, status, err := h.stripPrefix(r.URL.Path)
	if err != nil {
		return status, err
	}
...
func (h *Handler) handlePropfind(w http.ResponseWriter, r *http.Request) (status int, err error) {
	...
	if depth == 1 {
		...
		for _, e := range entries {
			info, _ := e.Info()
			body = funcName(body, info, "")

		}
	} else {
		body = funcName(body, fi, "")
	}
	body += `</D:multistatus>`
...
func funcName(body string, info fs.FileInfo, reqPath string) string {
	if reqPath == "/" {
		reqPath = ""
	}
	name := info.Name()
...
func (h *Handler) handleProppatch(w http.ResponseWriter, r *http.Request) (status int, err error) {
	...
	//body := `<?xml version="1.0" encoding="UTF-8"?>
	//<D:multistatus>
	//<D:response><D:href>/webdav-browser-extension.crx</D:href>
	//<D:propstat>
	//<D:prop>
	...
	body := `<?xml version="1.0" encoding="UTF-8"?>
<D:multistatus>
<D:response><D:href>` + reqPath + `</D:href>
<D:propstat>
<D:prop>
</D:prop>
<D:status>HTTP/1.1 403 Forbidden</D:status>
